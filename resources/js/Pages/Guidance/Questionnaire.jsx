...
export default function Questionnaire({ criteria = [] }) {
    ...
    const removeQuestion = async (criteriaId, index) => {
        ...
        setCriteriaList(
            criteriaList.map((c) => {
                if (c.id === criteriaId) {
                    const updatedQuestions = c.questions.filter(
                        (_, i) => i !== index
                    );
                    return { ...c, questions: updatedQuestions };
                }
        ...
        setNotification({
            type: "warning",
            message: "Question deleted. You can undo.",
        });
    ...
    const handleUndoQuestion = async () => {
        ...
        setNotification({
            type: "success",
            message: "Last deleted question restored.",
        });
        ...
        try {
            ...
            setCriteriaList([
                ...criteriaList,
                {
                    id: newCriteriaId,
                    title: newCriteria.title,
                    questions: newCriteria.questions,
                },
            ]);
    ...
    const saveQuestions = async (criteriaId) => {
        const criteria = criteriaList.find((c) => c.id === criteriaId);
        const unsavedQuestions = criteria.questions
            .filter((q) => !q.id && q.text.trim() !== "")
            .map((q) => q.text);
        ...
        try {
            const response = await axios.post(
                `/criteria/${criteriaId}/questions`,
                {
                    questions: unsavedQuestions,
                }
            );

            const savedQuestions = response.data.questions; // Assuming you modify your backend to return saved questions with IDs

            // Replace unsaved questions with saved ones
            const updatedQuestions = criteria.questions.map((q) => {
                if (!q.id && q.text.trim() !== "") {
                    const saved = savedQuestions.shift();
                    return saved;
                }
                return q;
            });
    ...
    const saveEditedQuestions = async (criteriaId) => {
        ...
        try {
            for (const q of criteria.questions) {
                if (q.id && typeof q.id === "number" && q.text.trim() !== "") {
                    await axios.put(`/questions/${q.id}`, {
                        text: q.text,
                    });
                }
            }
            ...
            <div className="relative p-6 space-y-6">
                ...
                <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button
                            onClick={() =>
                                router.visit("/guidance/questionnaireform")
                            }
                        >
                            Edit Order
                        </Button>
                ...
                {addingCriteria && (
                    <Card>
                        <CardHeader>
                            <Input
                                type="text"
                                placeholder="New Criteria Title"
