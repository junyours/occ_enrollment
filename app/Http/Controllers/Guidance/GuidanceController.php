...
class GuidanceController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validated = $request->validate(
            ...
            [
                'title.unique' => 'This criteria title already exists. Please use a different title.',
            ]
        );

        $lastPosition = Criteria::max('position') ?? 0;
        ...
        if (!empty($validated['questions'])) {
            foreach ($validated['questions'] as $q) {
                if (trim($q) !== '') {
                    $criteria->questions()->create(['text' => $q]);
                }
            }
        }
    ...
    public function save(Request $request)
    {
        $validated = $request->validate(
            ...
            [
                'title.unique' => 'This criteria title already exists.',
            ]
        );
        ...
        $criteria = Criteria::create([
            ...
            'position' => (Criteria::max('position') ?? 0) + 1,
        ]);
    ...
    {
        $criteria = Criteria::with('questions')
    ...
    public function bulkDownloadFacultyEvaluations(Request $request)
    {
        $activeEval = DB::table('evaluation as e')
            ->join('school_years as sy', 'e.school_year_id', '=', 'sy.id')
            ->join('semesters as s', 'sy.semester_id', '=', 's.id')
        ...
        $facultyList = DB::table('faculty as f')
            ->join('users as u', 'f.faculty_id', '=', 'u.id')
            ->join('user_information as ui', 'u.id', '=', 'ui.user_id')
            ->leftJoin('department as d', 'f.department_id', '=', 'd.id')
            ->join('year_section_subjects as yss', 'u.id', '=', 'yss.faculty_id')
        ...
        if ($facultyList->isEmpty()) {
            return back()->with('error', 'No faculty found for the selected filters.');
        }

        $tempDir = storage_path('app/temp/faculty-evaluation-zips');

        File::ensureDirectoryExists($tempDir);

        $zipFileName = 'faculty-evaluation-results-' . now()->format('Y-m-d-His') . '.zip';
        ...
        foreach ($facultyList as $faculty) {
            ...
            foreach ($subjects as $subject) {
                $data = $this->buildFacultyEvaluationPdfData(
                    $faculty->id,
                ...
                $pdf = Pdf::loadView('pdf.faculty-evaluation-result', $data)
                    ->setPaper('a4', 'portrait');

                $departmentFolder = Str::slug(
                    $faculty->department_name ?: 'No Department'
                );

                $instructorFolder = Str::slug(
                    $faculty->last_name . '-' . $faculty->first_name . '-' . $faculty->id
                );

                $subjectFolder = Str::slug(
                    $subject->subject_code . '-' . $subject->descriptive_title . '-' . $subject->subject_id
                );

                $pdfFileName = Str::slug($faculty->last_name, '_')
                    . '_'
                    . Str::slug($faculty->first_name, '_')
                    . '_'
                    . Str::slug($subject->subject_code . '_' . $subject->descriptive_title, '_')
                    . '.pdf';

                $zipEntryName = $departmentFolder . '/'
                    . $instructorFolder . '/'
                    . $subjectFolder . '/'
                    . $pdfFileName;

                $zip->addFromString($zipEntryName, $pdf->output());
        ...
        if ($filesAdded === 0) {
            ...
            return back()->with('error', 'No evaluation results found to download.');
        }
    ...
    private function buildFacultyEvaluationPdfData($facultyId, $studentSubjectId)
    {
        ...
        $evaluation = DB::table('evaluation as e')
            ...
            ->where('e.status', 'active')
            ->latest('e.start_date')
            ->select('e.*', 'sy.id as sy_id', 'sy.start_year', 'sy.end_year', 'sem.semester_name')
            ->first();
        ...
        $relatedStudentSubjectIds = DB::table('student_subjects as ss')
            ...
            ->where('ys.school_year_id', $evaluation->sy_id)
            ->where('ss.dropped', 0)
            ->pluck('ss.id');

        if ($relatedStudentSubjectIds->isEmpty()) {
            return null;
        }
        ...
        $overallAverage = $criteriaGrouped->isNotEmpty()
            ? round($criteriaGrouped->avg('average'), 2)
            : null;
        ...
        return [
            ...
            'isValidEvaluation' => $isValidEvaluation,
            'schoolYear' => $schoolYear,
            'feedback' => $feedback,
    ...
    public function delete($type, $id)
    {
        ...
        if ($type === 'criteria') {
            ...
            if ($isUsed) {
                return Inertia::location(
                    url()->previous() . '?message=' . urlencode('Cannot delete criteria — it is used in an evaluation.') . '&type=error'
                );
            }
        }
        ...
        if ($type === 'question') {
            ...
            if ($isUsed) {
                return Inertia::location(
                    url()->previous() . '?message=' . urlencode('Cannot delete question — it is used in an evaluation.') . '&type=error'
                );
            }
        }

        // ------------------------------------
        // 🗑 Safe delete (Criteria/Question not used)
        // ------------------------------------
        $item->forceDelete();
    ...
    public function deleteAll(Request $request)
    {
        ...
        if ($type === 'all') {
            ...
            // Delete Questions only if NOT used in evaluations
            $questions = Question::onlyTrashed()->get();
            foreach ($questions as $question) {
                $isUsed = DB::table('evaluation_questions')
            ...
            // Delete Criterias only if NOT used in evaluations
            ...
            // Evaluations have no dependencies → always safe
        ...
        if ($type === 'question') {
            ...
            return Inertia::location(
                url()->previous() . '?message=Questions+deleted+where+safe&type=success'
            );
        }
    ...
    {
        return Inertia::render('Guidance/FesFacultyResult/FacultyFesReport', [
